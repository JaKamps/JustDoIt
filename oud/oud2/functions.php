...
function connectList($conn, $name) {
    $sql = "select * from lijsten where naamLijst = ?";
    $stmt = mysqli_stmt_init($conn);
    if (!mysqli_stmt_prepare($stmt, $sql)) {
        header("location: SignUp.html?error=stmtfailed");
        exit();
...
function getList($conn, $idUsers) {
    $sql = "SELECT lijsten.idLijsten, lijsten.naamLijst 
    FROM ((users_has_lijsten
    INNER JOIN users ON users_has_lijsten.Users_idUsers = users.idUsers)
    INNER JOIN lijsten ON users_has_lijsten.Lijsten_idLijsten = lijsten.idLijsten)
    WHERE users.idUsers = ?";
...
function deleteList($conn, $id) {
    disableChecks($conn);
    $sql = "DELETE lijsten, users_has_lijsten FROM lijsten, users_has_lijsten
    WHERE lijsten.idLijsten = users_has_lijsten.Lijsten_idLijsten and lijsten.idLijsten = ?";
    $stmt = mysqli_stmt_init($conn);
    if (!mysqli_stmt_prepare($stmt, $sql)) {
        header("location: SignUp.html?error=stmtfailed");
        exit();
